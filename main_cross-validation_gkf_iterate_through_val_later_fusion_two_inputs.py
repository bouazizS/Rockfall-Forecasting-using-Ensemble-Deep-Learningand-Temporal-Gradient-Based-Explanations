...
from gradcam_Inception.inception_time_multi import InceptionTimeModel #, inception_time

#from gradcam_Inception.inception_attention import InceptionTimeModel #, inception_time

from gradcam_Inception.gradcam import GradCAM ,plot_with_heatmap_orig, plot_with_heatmap
from gradcam_Inception.utils import  process_rockfall_data_multi,plot_save_conf_matrix, process_rockfall_data, charge, process_rockfall_data_year, plot_training_curves_gkf, preprocess_segments, plot_PR_gkf_per_year_val
...
#pour convertir les ndarrays en listes
...
#load df of rr1, temp and sismo events
...
for hp in [3, 7]:
    path= f'testing_models_gkf_multi_branch/Rain_H_all_models_Shuffle/HP_{hp}/'
    path_curves = f'testing_models_gkf_multi_branch/Rain_H_all_models_Shuffle/HP_{hp}/curves/'
    ...
    X_1 = df[[f"RR1_hour_{i}" for i in range(336)]].values  #----> colonnes de précipitations
    X_2 = df[[f"H_hour_{i}" for i in range(336)]].values  #----> colonnes de charge/discharge

    x1_scaled, scaler1 = preprocess_segments(X_1) 
    x2_scaled, scaler2 = preprocess_segments(X_2)    #x_scaled de shape (n_samp, 336, 1) 
    ...
    x1_tensor = torch.FloatTensor(x1_scaled).permute(0, 2, 1)   # de shape (n_samp, 1,336)
    x2_tensor = torch.FloatTensor(x2_scaled).permute(0, 2, 1)   # de shape (n_samp, 1,336)
    ...
    for fold, (train_val_idx, test_idx) in enumerate(gkf.split(x1_tensor, y_encoded, groups=groups)):
        ...
        for val_year in remaining_years :
            ...
            if use_attention :  # model with attention
                ...
                model = InceptionTimeModel( in_channels_1=1, in_channels_2=1, num_classes=len(np.unique(y_encoded)), n_blocks=2, merge_mode='concat')
            ...
            for epoch in range(nb_epochs):
                ...
                for (data1,data2), target in train_loader:  #now we take (x1,x2) from data
                    optimizer.zero_grad()
                    #Pass bith inputs to the model
                    ...
                    #save les vraies classes et les prédictions
                    all_train_targets.extend(target.cpu().numpy())
                    all_train_predictions.extend(predicted.cpu().numpy())


                #métriques pour le training set
                ...
                train_precision_class_0.append(precision_train_per_class[0])
                train_recall_class_0.append(recall_train_per_class[0])
                train_precision_class_1.append(precision_train_per_class[1])
                train_recall_class_1.append(recall_train_per_class[1])
                '''
                ...
                if val_loss < best_val_loss:
                    ...
                    epochs_no_improve = 0
                    #track overall best across all val years
                    if val_loss < best_overall_val_loss:
                        best_overall_val_loss = val_loss
                        best_overall_model = model
                        best_val_year = val_year
                        best_overall_state = model.state_dict()
                else:
                    epochs_no_improve += 1  

                #if nombre d'epochs sans amelioration depasse patience
            ...
            #save each model (parmis les 9 modeles) at each fold of test year 
            ...
            train_metrics = {
                ...
                'train_precision_class_0': train_precision_class_0,
                'train_precision_class_1': train_precision_class_1,
                'train_recall_class_0': train_recall_class_0,
                'train_recall_class_1': train_recall_class_1,
                'num_epochs_trained': num_epochs_trained
            }
            all_train_metrics[f"val_year{val_year}"] = train_metrics
            '''
            ...
            #plot_save_conf_matrix(conf_matrix, [0,1],  test_year_folder)
        ...
        #for saving the best model across 9 models
        #if best_overall_model is not None:
            # model_path = f"{path}best_model_HP_{hp}_testyear_{test_year[0]}_val_{best_val_year}.pt"
            # #torch.save( best_overall_state, model_path)
            # torch.save({
            #     'model_state_dict': best_overall_state,
            #     'best_val_year': best_val_year,
            #     'best_val_loss': best_overall_val_loss
            # }, model_path)

        #avreaged metrics across all 9 models for each test year
        avg_metrics = {
            'test_year': test_year,
            ...
            #'all_val_splits': val_split_results
        }
        
        all_test_results.append(avg_metrics)


    #conversion de all_test_results before save 
